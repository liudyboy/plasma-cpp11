...
void add_mock_object_conn(plasma_mock *mock, ObjectId oid) {
  /* Connect a new client to the local plasma manager and mock a request to an
   * object. */
  mock->plasma_conn =
...
TEST request_transfer_test(void) {
  ...
  const char **manager_vector = (char**)malloc(sizeof(char *));
  ...
  ASSERT(memcmp(&oid, &req->object_ids[0], sizeof(ObjectId)) == 0);
  /* Clean up. */
  free(req);
...
TEST request_transfer_retry_test(void) {
  ...
  const char **manager_vector = (char**)malloc(sizeof(char *) * 2);
  manager_vector[0] = "127.0.0.1:12346";
...
TEST request_transfer_timeout_test(void) {
  ...
  ASSERT_EQ(memcmp(&oid, &reply.object_id, sizeof(ObjectId)), 0);
...
TEST read_write_object_chunk_test(void) {
  ...
  plasma_request_buffer local_buf = {
      ...
      .data_size = data_size,
      .metadata_size = metadata_size,
      .data = (u_int8_t*)malloc(data_size),
  };
  /* The test:
