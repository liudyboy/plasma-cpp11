CC = g++
CFLAGS = -g -Wall --std=c++17 -D_XOPEN_SOURCE=500 -D_POSIX_C_SOURCE=200809L -I. -Icommon -Icommon/thirdparty
...
$(BUILD)/plasma_client.so: src/plasma_client.c src/fling.h src/fling.c common src/local_store.c
	$(CC) $(CFLAGS) src/plasma_client.c src/fling.c common/build/libcommon.a src/local_store.c -fPIC -shared -o $(BUILD)/plasma_client.so

$(BUILD)/libplasma_client.a: src/plasma_client.o src/fling.o common/io.o src/local_store.o
	ar rcs $@ $^
...
put: src/plasma_client.c src/plasma.h test/put.c src/fling.h src/fling.c common
	$(CC) $(CFLAGS) src/plasma_client.c test/put.c src/fling.c common/build/libcommon.a -o $(BUILD)/put
