...
int plasma_request_size(int num_object_ids) {
  int object_ids_size = (num_object_ids - 1) * sizeof(ObjectId);
...
plasma_request make_plasma_request(ObjectId object_id) {
...
plasma_request *make_plasma_multiple_request(int num_object_ids,
  ...
  plasma_request *req = (plasma_request*)malloc(req_size);
  req->num_object_ids = num_object_ids;
  memcpy(&req->object_ids, object_ids, num_object_ids * sizeof(ObjectId));
  return req;
}
...
void increment_object_count(plasma_connection *conn,
  ...
  HASH_FIND(hh, conn->objects_in_use, &object_id, sizeof(ObjectId),
            object_entry);
  if (object_entry == NULL) {
    ...
    HASH_ADD(hh, conn->objects_in_use, object_id, sizeof(ObjectId),
             object_entry);
...
plasma_connection *plasma_connect(const char *store_socket_name,
  ...
  plasma_connection *result = (plasma_connection*)malloc(sizeof(plasma_connection));
...
void plasma_disconnect(plasma_connection *conn) {
  plasma_request req = {};
  plasma_send_request(conn->store_conn, DISCONNECT_CLIENT, &req);
...
void plasma_fetch(plasma_connection *conn,
  ...
  for (int received = 0; received < num_object_ids; ++received) {
    ...
    for (; i < num_object_ids; i++) {
      if (memcmp(&object_ids[i], &reply.object_id, sizeof(ObjectId)) == 0) {
