...
void *fake_mmap(size_t size) {
  ...
  struct mmap_record *record = (struct mmap_record*)malloc(sizeof(struct mmap_record));
  record->fd = fd;
  ...
  pointer =  (void*)((size_t)pointer + sizeof(size_t));
...
void get_malloc_mapinfo(void *addr,
  ...
  for (record = records_by_fd; record != NULL; record = (struct mmap_record*)record->hh_fd.next) {
    if (addr >= record->pointer && (size_t)addr < (size_t)record->pointer + record->size) {
      *fd = record->fd;
      *map_size = record->size;
      *offset = (size_t)addr - (size_t)(record->pointer);
      return;
    }
  }
  *fd = -1;
