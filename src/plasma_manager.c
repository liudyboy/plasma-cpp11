...
client_object_connection *add_object_connection(client_connection *client_conn,
  ...
  client_object_connection *object_conn =
      (client_object_connection*)malloc(sizeof(client_object_connection));
  if (!object_conn) {
    return NULL;
  }
  ...
  HASH_ADD(active_hh, client_conn->active_objects, object_id, sizeof(ObjectId),
           object_conn);
  ...
  HASH_FIND(fetch_hh, client_conn->manager_state->fetch_connections, &object_id,
            sizeof(ObjectId), fetch_connections);
...
client_connection *get_manager_connection(plasma_manager_state *state,
  ...
  if (!manager_conn) {
     ...
     * unsuccessful. */
    CHECK(fd >= 0);
    manager_conn = (client_connection*)malloc(sizeof(client_connection));
    manager_conn->fd = fd;
...
int manager_timeout_handler(event_loop *loop, timer_id id, void *context) {
  client_object_connection *object_conn = (client_object_connection*)context;
...
void request_transfer(ObjectId object_id,
  ...
  object_conn->manager_vector = (char**)malloc(manager_count * sizeof(char *));
  memset(object_conn->manager_vector, 0, manager_count * sizeof(char *));
  for (int i = 0; i < manager_count; ++i) {
    int len = strlen(manager_vector[i]);
    object_conn->manager_vector[i] = (char*)malloc(len + 1);
